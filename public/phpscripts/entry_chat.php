<?php
    ...
    if(!$stmnt = $connection->prepare('SELECT username , userid from users where userid in (select send_id from messages where rcv_id = ? )')){
    ...
    $userFriendsID = array();
    $friend_id = -1;
    if(!$stmnt->bind_result($user,$friend_id)){
        die(json_encode(array(
            'status'=>'error',
            'msg'=>'binding failed'
        )));
