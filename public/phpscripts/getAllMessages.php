...
require 'db_connect.php';
    ...
    $stmnt2 = $connection->prepare('SELECT body FROM messages where send_id = ? and rcv_id = ? and msgTime <= FROM_UNIXTIME(?) ORDER BY msgTime DESC LIMIT 10');
    if(!$stmnt2) {
        echo "hello there";
        die(json_encode(array(
    ...
    $latestmessage = '';
    if(!$stmnt2->bind_param('iii',$senderID,$recieverID, $time)){
        die(json_encode(array(
            'status'=>'error in second part',
